...
function Admin() {
    ...
    const getMovies = () => {
    fetch("https://bookmyshow-msnp.onrender.com/bookmyshow/movies",
    {
        method:"GET",
    headers:{
        Authorization:token
    ...
    const deleteList = (id) => {
        ...
        fetch(`https://bookmyshow-msnp.onrender.com/bookmyshow/movies/${id}`,{
          method:"DELETE"
        })
        .then(() => getMovies())
         }
...
{
    movies.map((e,i) => <Movielist movie={e} key={i} id={e.id} deletebutton = {<DeleteIcon onClick={() => deleteList(e.id)}/>} />)
}
  </Box>
  </>
}
